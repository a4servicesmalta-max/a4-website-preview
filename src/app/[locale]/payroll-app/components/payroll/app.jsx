...
import { EmployeeDetail, PeopleView } from "./views-people.jsx";
import { PaySidebar, PayTopbar } from "./ui.jsx";
import { WizardModal } from "./wizard.jsx";
...
export function PayrollApp() {
  const persisted = (() => {
    try {
      return JSON.parse(localStorage.getItem(PAY_LS) || "{}");
    } catch {
      return {};
    }
  })();
  const [employees, setEmployees] = useState(persisted.employees || PAY_SEED);
  ...
  const [monthIdx, setMonthIdx] = useState(persisted.monthIdx ?? 0);

  useEffect(() => {
    try {
      localStorage.setItem(PAY_LS, JSON.stringify({ employees, runs, monthIdx }));
    } catch {
      /* ignore */
    }
        ...
        <main className="pay-scroll" style={{ flex: 1, overflowY: "auto", padding: "26px 28px 48px" }}>
          ...
          {page === "people" && emp && (
            <EmployeeDetail
              ...
              runs={runs}
              back={() => setEmpId(null)}
              openPayslip={(r, e) => setSlip({ runId: r, empId: e })}
            />
          )}
          {page === "run" && (
            <RunPayrollView employees={employees} runs={runs} monthIdx={monthIdx} setMonthIdx={setMonthIdx} approve={approve} />
          )}
