...
function PMScreenAccount({ reduce }: { reduce: boolean }) {
  ...
  return (
    <div className="pt-[26px] px-[26px] pb-[24px]">
      ...
      <div className="flex flex-col gap-[13px] mt-[22px]">
        {fields.map((f, i) => (
          <div key={f.label} style={pmRise(2 + i, reduce)}>
            ...
            <div className="flex items-center h-[44px] px-[14px] rounded-[var(--a4-r-md)] bg-[var(--a4-surface-deep)] border border-[var(--a4-hairline-dark)] a4-font-body text-[14px] text-[#fff]">
              {f.value}
            </div>
          </div>
        ))}
      ...
      <div className="text-center mt-[14px] a4-font-body text-[11.5px] text-[var(--a4-stone)]" style={pmRise(5, reduce)}>No card required · ready in 2 minutes</div>
    </div>
  );
}
  ...
  return (
    <div className="pt-[26px] px-[26px] pb-[24px]">
      ...
      <div className="mt-[18px]" style={pmRise(4, reduce)}>
        <div className="flex items-center justify-center gap-[8px] h-[46px] rounded-[var(--a4-r-full)] bg-[var(--a4-primary)] text-[#fff] a4-font-body text-[14.5px] font-semibold">
...
export function PortalMockup() {
  ...
  return (
    <div id="a4-portal-mock" className="relative w-full max-w-[480px]">
      <div className="absolute -top-[10%] -bottom-[16%] -left-[6%] -right-[6%] blur-[22px] pointer-events-none" style={{ background: "radial-gradient(58% 52% at 52% 34%, rgba(73,79,223,.18), transparent 72%)" }} />
      <div className="relative bg-[var(--a4-surface-elevated)] border border-[var(--a4-hairline-dark)] rounded-[var(--a4-r-xl)] overflow-hidden shadow-[0_30px_80px_-30px_rgba(0,0,0,.9)]">
        {/* window chrome */}
        <div className="flex items-center gap-[8px] py-[13px] px-[16px] border-b border-[var(--a4-divider-soft)]">
          <span className="w-[10px] h-[10px] rounded-full bg-[#3a3d40]" />
          <span className="w-[10px] h-[10px] rounded-full bg-[#3a3d40]" />
          <span className="w-[10px] h-[10px] rounded-full bg-[#3a3d40]" />
          <div className="flex-1 text-center a4-font-body text-[11.5px] text-[var(--a4-stone)] tracking-[.4px]">client.a4.com.mt</div>
        </div>
          ...
          {PM_STEPS.map((label, i) => (
            <div key={label} className="flex-1">
              <div className="flex items-center gap-[7px] mb-[8px]">
                <span className="w-[18px] h-[18px] rounded-full grid place-items-center shrink-0 a4-font-body text-[10.5px] font-bold transition-colors duration-300" style={{ background: i <= shown ? "var(--a4-primary)" : "var(--a4-surface-deep)", color: i <= shown ? "#fff" : "var(--a4-stone)", border: i <= shown ? "none" : "1px solid var(--a4-hairline-dark)" }}>{i + 1}</span>
                <span className="pm-steplabel a4-font-body text-[11.5px] whitespace-nowrap transition-colors duration-300" style={{ fontWeight: i === shown ? 600 : 500, color: i === shown ? "#fff" : "var(--a4-stone)" }}>{label}</span>
              </div>
              <div className="h-[3px] rounded-full bg-[var(--a4-surface-deep)] overflow-hidden">
                <div key={`${i}-${shown}-${started}`} className="h-full rounded-full bg-[var(--a4-primary)]" style={{
