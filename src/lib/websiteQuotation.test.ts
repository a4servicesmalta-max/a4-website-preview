...
describe("per-item pricing", () => {
  it("prices managed bookkeeping by entity and expenses, with no risk uplift", () => {
    // mt-2026-08-27-entry: the first bank account is INCLUDED, so at one
    // account the monthly is the bare base — 24 sole, 49 company.
  ...
  it("prices only accounts beyond the first, the first included in the fee, rounded per account THEN multiplied (mt-2026-08-27-entry)", () => {
    // The owner's worked example: company 10–25k at 21–60 → 69 + 10 = 79;
    // per account round(40 + 0.15 × 79) = 52; two extra accounts 104; full 183.
    ...
    expect(t.lines).toEqual([
      ...
      { label: "Additional bank accounts · 2 × €52", amount: 104, cadence: "monthly" },
    ]);
    expect(t.grossMonthly).toBe(183);
    // At one account there is NO bank line at all — it is included.
    const one = evaluateA4Items([{ service: "bookkeeping-managed", entity: "company", expenses: "0-10k", txn: "1-20", banks: 1 }], "standard", AFTER);
    expect(one.lines.map((l) => l.label)).toEqual(["Managed bookkeeping — Company"]);
    // Rounding order is pinned: 2 × round(47.35) = 94, NOT round(2 × 47.35) = 95.
    expect(banksMonthly("company", "0-10k", "1-20", 3)).toBe(94);
    expect(gross([{ service: "bookkeeping-managed", entity: "company", expenses: "0-10k", txn: "1-20", banks: 3 }]).monthly).toBe(49 + 94);
  ...
  it("charges catch-up at the monthly rate, per month, with no cap", () => {
    ...
    // Catch-up = months × the FULL monthly, extra bank accounts included
    // (mt-2026-08-27-entry): the worked example's 183/mo × 12 = 2196.
    expect(gross([{ service: "catchup", months: 12, entity: "company", expenses: "10-25k", txn: "21-60", banks: 3 }]).oneOff).toBe(12 * 183);
    expect(catchUpLabel(12, "company", "10-25k", "21-60", 3)).toBe("Catch-up: 12 months x EUR 183 = EUR 2196");
  ...
  it("drops unpriceable items rather than sinking them into the totals", () => {
    ...
    expect(t.lines).toHaveLength(1); // the base line only (one account is included); the payroll item is gone
...
describe("bookkeeping by monthly expenses (pack mt-2026-08-14-volume)", () => {
  ...
  it("prices every band, for both entities, exactly as the contract says", () => {
    // The TABLE is the BASE rate, and at one account (included since
    // mt-2026-08-27-entry) the monthly IS the base.
    for (const band of EXPENSE_BANDS) {
      for (const entity of ["sole", "company"] as const) {
        ...
        expect(`${entity}/${band.id} = ${got}`).toBe(`${entity}/${band.id} = ${base}`);
      }
    }
  });
  ...
  it("prices the top band instantly — 500k+ is a real price, not a 'talk to us'", () => {
    ...
    expect(t.grossMonthly).toBe(549); // the one bank account is included
  ...
  it("charges a backdated month at the client's OWN rate — worked example at €25–50k", () => {
    // A company at the 25-50k band pays €99/mo (its one account included), so
    // 9 backdated months is 9 × 99 = 891 — NOT 9 × 49 = 441, which is what
  ...
  it("prices a sole trader's catch-up off the sole table, at a non-entry band", () => {
    // 100-200k sole = €129/mo, one account included. 6 months = 774.
  ...
  it("keeps the other services on their own drivers, untouched by the bookkeeping band", () => {
    ...
    expect(withTopBand.monthly).toBe(baseline.monthly + 549); // its one account included
...
describe("mixed basket, hand-computed", () => {
  ...
  it("totals every cadence correctly before the promo", () => {
    ...
    // Monthly: bookkeeping 49 (flat, NO uplift; its one account included)
    //        + VAT      69 × 1.2 = 82.8 → 83
    ...
    expect(t.hasUnpricedOnboarding).toBe(true);
    expect(t.registryPassThrough).toBe(210);
    // Seven items in, six priced lines out — onboarding emits none and
    // bookkeeping emits one (its single account is included).
...
describe("call-site baskets", () => {
  ...
  for (const [label, items] of Object.entries(baskets)) {
    it(`submits what it displays — ${label}`, () => {
      ...
      // These baskets are all single-account, so bookkeeping emits exactly
      // one line (mt-2026-08-27-entry: the first account is included).
